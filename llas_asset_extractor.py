...
def find_pack_file(base_dir, pack_name):
    """Find the actual pack file path, autodetecting several layouts.

    - elichika  : static/<pack_name>
    - elichika  : static/pkg<X>/<pack_name>   (some setups)
    - game dump : pkg<X>/<pack_name>          (original LLASDecryptor layout)
    - flat      : <base_dir>/<pack_name>
    """
    if not pack_name:
        return None
    first = pack_name[0]
    candidates = [
        os.path.join(base_dir, "static", pack_name),
        os.path.join(base_dir, "static", "pkg" + first, pack_name),
        os.path.join(base_dir, "pkg" + first, pack_name),
...
def mode_costume(base_dir, out_dir, asset_db):
    ...
    for n in chosen:
        ...
        if extract_one(base_dir, out_dir, "member_model", label, pack_name,
...
def mode_table(base_dir, out_dir, asset_db):
    ...
    for n in chosen:
        ...
        if extract_one(base_dir, out_dir, table, ap, pn, hd, sz, k1, k2, used, manifest):
            ok += 1
    if manifest:
        write_manifest(out_dir, manifest)
    print(f"\nDone. {ok} ok / {len(chosen)} selected")
...
def main():
    ...
    if mode == 1:
        mode_costume(base_dir, out_dir, asset_db)
    else:
        mode_table(base_dir, out_dir, asset_db)
