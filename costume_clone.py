...
def get_costume_selection(costumes, costume_dict, dict_cursor):
    while True:
...
def get_no_mask_model_path(cursor, suit_id):
    """Get model_asset_path for no-mask version from m_suit_view"""
    cursor.execute("SELECT model_asset_path FROM m_suit_view WHERE suit_master_id = ?", (suit_id,))
...
cid, key, rname = get_costume_selection(rows, cdict, dict_cursor)

# Rina (209) mask selection
use_no_mask = False
if src_id == '209':
    print("\n" + "="*80)
    ...
    print("="*80)
    mask_choice = input("Select version (1 or 2): ").strip()
    if mask_choice == '2':
        use_no_mask = True
        print("✅ Selected: Without mask version")
    else:
        print("✅ Selected: With mask version (default)")
...
# Get model_asset_path based on mask selection
model_path = mrow[4]  # Default: with mask
if use_no_mask:
    no_mask_path = get_no_mask_model_path(md_cur, cid)
    if no_mask_path:
        model_path = no_mask_path
        print(f"✅ Using no-mask model path: {model_path}")
    else:
        print("⚠️  No-mask model path not found, using default model path")
...
, (new_id, tgt_id, cloned_key, mrow[1], mrow[2], mrow[3], model_path, new_display))
md_conn.commit()
...
# Get model_asset_path for JP database
model_path_jp = mrow[4]  # Default: with mask
if use_no_mask:
    no_mask_path_jp = get_no_mask_model_path(md_cur, cid)
    if no_mask_path_jp:
        model_path_jp = no_mask_path_jp

new_display = md_cur.execute("SELECT MIN(display_order) FROM m_suit WHERE member_m_id = ?", (tgt_id,)).fetchone()[0] - 1
md_cur.execute("INSERT INTO m_suit (id, member_m_id, name, thumbnail_image_asset_path, suit_release_route, suit_release_value, model_asset_path, display_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
, (new_id, tgt_id, cloned_key, mrow[1], mrow[2], mrow[3], model_path_jp, new_display))
md_conn.commit()
