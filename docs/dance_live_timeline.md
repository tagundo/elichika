...
5. **`m_live.is_2d_live = 0` 플립(중요)** — SIFAS는 이 플래그가 **0일 때만 3D 댄스를
   렌더**합니다. 오디오/2D-PV로 출시된 곡은 `is_2d_live = 1`이라, 타임라인·`m_live_3d_asset`을
   아무리 연결해도 **게임에선 계속 2D로** 뜹니다. 그래서 설치기는 대상 곡의 이 컬럼을
   0으로 바꿉니다(컬럼이 있을 때만, 스키마-불문). 고급 용도로 끄려면 `--keep-2d`.
   - 근거: `live_addon_installer.py`가 2D-PV 라이브를 만들 때 `m_live`에
     `is_2d_live = '1'`로 넣습니다(= 1이 2D, 0이 3D). 서버는 `gamedata/live.go`에서 이
     컬럼을 읽지 않고(주석 처리), **클라이언트가 내려받은 masterdata로 2D/3D를 결정**합니다.
...
### 곡 목록의 "읽는 이름"
`m_live.name`은 **사전 키**(예: `k.live_name_...`)이고, 사람이 읽는 곡 제목은
`dictionary_<lang>_k.db`의 `m_dictionary.message`입니다(서버도 동일:
`live.Name = Dictionary.Resolve(live.Name)`). 도구의 곡 목록은 이제 `db/gl|jp`의 사전
DB들(en→ko→zh→ja 순)을 자동으로 찾아 **실제 곡 제목**으로 표시하고, 가나 발음
(`pronunciation`)이 있으면 괄호로 덧붙입니다.
...
### 의상 설정/대기 화면에 진입조차 못 해요 (솔로곡)
**원인**: 대상이 **솔로(1인) 곡**인데 그 멤버의 `m_live_member_mapping.suit_master_id`가
**NULL**이면, 3D는 그 한 명의 모델을 이 suit에서 불러오므로 **불러올 의상이 없어 화면을
구성하지 못합니다**. 2D 전용 솔로 캐릭터송을 3D로 바꿀 때 전형적으로 발생(출시된 솔로 3D
라이브는 전부 suit가 채워져 있음). 설치기가 이제 **자동으로** 그 멤버의 유효한 3D 의상을
채웁니다(요약 `solo_suit_set`). 특정 의상을 강제하려면 `--solo-suit <id>`, 끄려면
`--no-suit-fix`. 수동으로는:
```sql
UPDATE m_live_member_mapping SET suit_master_id = <코토리 3D 의상 id>
WHERE mapping_id = <live_member_mapping_id> AND position = 1;
```
유효한 의상은 `m_suit`에서 `member_m_id`가 그 멤버이고 `model_asset_path`가 있는 값.

### 의상 화면은 뜨는데 MV가 로딩이 안 돼요 (`m_live_mv`)
**원인**: 3D MV는 `m_live_mv`(곡별 **stage·position 목록**)를 읽어 무대를 구성하는데,
2D였던 곡엔 이게 **0행**이라 MV가 보여줄 게 없어 로딩이 멈춥니다. 출시된 MV 곡은 전부
**3 position**을 가집니다(p1=stage 1000702, p2=1000801, p3=곡 고유 stage). 설치기가 이제
**자동으로** 같은 member_group 템플릿에서 3 position을 복사하고, p3는 그 곡 3D 에셋의
stage로 맞춥니다(요약 `live_mv_positions_added`). 끄려면 `--no-mv-fix`. 수동으로는(예:
3D 에셋 id가 `127315551`, 곡 stage가 `1000601`인 솔로):
```sql
INSERT INTO m_live_mv (live_id, live_stage_master_id, position, live_3d_asset_master_id)
SELECT <대상 live_id>, live_stage_master_id, position, <3D에셋 id>
FROM m_live_mv WHERE live_id = 12002;   -- 정상 솔로곡을 템플릿으로 복사
```

### 설치했는데 게임에선 아직 2D로 떠요 (체크리스트)
1. **`m_live.is_2d_live = 0` 인가** — 가장 흔한 원인. 위 §5 플립이 되어야 3D로 렌더됩니다.
   요약에 `is_2d_live_set_to_3d: True`가 떴는지 확인. (`--keep-2d`로 껐다면 켜서 재설치)
2. **난이도 재연결** — `m_live_difficulty.live_3d_asset_master_id`가 새 3D 에셋을 가리키는지.
   `add` 모드에서 일부 난이도만 선택했다면 **선택한 난이도에서만** 3D가 뜹니다.
3. **서버 재시작 + 클라 동기화** — masterdata를 고친 뒤 **서버를 재시작**해야 암호화된 DB가
   다시 생성되고, 클라이언트가 로그인/이동 시 새 DB를 내려받습니다(자세히는
   `docs/modify_database.md`). 이 단계를 빼면 클라가 옛 masterdata를 써서 계속 2D입니다.
4. **에셋 다운로드 가능 여부** — 타임라인/모션/표정이 CDN(`m_asset_package*`)에 등록되어
   실제로 내려받아져야 3D 씬이 구성됩니다(도구가 등록하지만, `cdn_server` 설정 확인).
